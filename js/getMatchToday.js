function getMatchTodayHTML(data) {
  ...
  data.matches.forEach((match, index) => {
    ...
    async function getImg(match) {
      try {
        ...
        const printMatch = document.getElementById("mt");
        printMatch.innerHTML += html;
        if (index <= 1) {
          const test = document.querySelector("#mt .hidden");
          // console.log(index);
  ...
  function loadMore() {
    const hidden = document.querySelector("#mt .hidden");
    if (hidden) {
